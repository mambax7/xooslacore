...
{
    $group =  new Xooslacore\XooslaPermissions();
...
function xoosla_getHtmlEditor()
{
    ...
    if ($use_wysiwyg !== null && in_array($use_wysiwyg, ['tinymce', 'ckeditor', 'koivi', 'inbetween', 'spaw'])) {
        return true;
    }
...
function xoosla_securityCheck()
{
    if (!$GLOBALS['xoopsSecurity']->check()) {
        $redirect = null !== Request::getString('HTTP_REFERER', '', 'SERVER') ? urldecode(Request::getString('HTTP_REFERER', '', 'SERVER')) : xoops_getenv('PHP_SELF');
        redirect_header($redirect, 1, _XL_AD_ADM_DBERROR);
    }
}

\XoopsModules\Xooslacore\Core\XooslaLoad::getInclude('modules.xooslacore.libraries.functions_admin');
